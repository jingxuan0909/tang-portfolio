...
// Guides the Admin through email request, OTP verification, and password update.
export function ResetPasswordPage() {
  const location = useLocation();
  // Session storage keeps recovery progress during a page refresh in this tab.
  ...
  // Accept a password change only after this browser tab verified a recovery OTP.
  useEffect(() => {
    let active = true;
  ...
  // Counts down before another recovery email can be requested.
  useEffect(() => {
  ...
  // Asks Supabase to email a random recovery code to the Admin account.
  async function requestCode(event) {
    event?.preventDefault();
    const normalizedEmail = email.trim();
    if (!normalizedEmail) { setMessage("Enter your Admin email first."); return; }
    setBusy(true); setMessage("");
    // The redirect returns link-based recovery emails to this same page.
  ...
  // Exchanges the six-digit recovery code for a temporary authenticated session.
  ...
  // Saves the new password, signs out, and removes temporary recovery markers.
  ...
  // Positive messages use the normal success color instead of the error color.
  ...
  return <main className="admin-login"><div className="admin-login__panel admin-recovery-panel">
    ...
    {/* Replace the form with a clear success state after the password is saved. */}
    {saved ? <div className="admin-recovery-success"><CheckCircle size={34} weight="fill" /><h2>Password updated</h2><p>Your old password is no longer valid. Sign in again with your new password and Gmail OTP.</p><Link className="button button--primary" to="/admin">Continue to Admin</Link></div> : <>
      ...
      {/* Step 1: request a code for the Admin email. */}
      ...
      {/* Step 2: verify the newest six-digit code. */}
      ...
      {/* Step 3: choose and confirm the replacement password. */}
