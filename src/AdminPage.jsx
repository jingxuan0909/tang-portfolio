...
function RecordEditor({ title, description, items, fields, createItem, onChange, setMessage, addLabel = "Add" }) {
  const [editor, setEditor] = useState(null);
  ...
  return <section className="admin-card"><div className="admin-card__heading"><div><h2>{title}</h2><p>{description}</p></div><button type="button" onClick={openAdd}><Plus /> {addLabel}</button></div>
    <div className="admin-entry-list">{items.map((item, index) => <article className="admin-entry" key={item.id || `${title}-${index}`}><div className="admin-entry__details">{fields.map((field, fieldIndex) => <span className={fieldIndex === 0 ? "admin-entry__primary" : ""} key={field.key}><small>{field.label}</small>{field.display ? field.display(item[field.key]) : item[field.key]}</span>)}</div><div className="admin-entry__actions"><button type="button" onClick={() => openEdit(index)}><PencilSimple /> Edit</button><button type="button" className="delete-button" onClick={() => remove(index)}><Trash /> Delete</button></div></article>)}</div>
