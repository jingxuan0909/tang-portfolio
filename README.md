...
```text
tang-portfolio/
|-- .env.example                Safe template for public Supabase environment variables
|-- .npmrc                      npm installation-output preferences
|-- AGENTS.md                   Durable implementation and product decisions
|-- data/                       Initial fallback portfolio content
|-- db/                         Legacy Sites database schema
|-- drizzle/                    Legacy Sites SQL migration
|-- public/
|   |-- assets/                 Website images and project logos
|   `-- uploads/                Initial award and certificate documents
|-- scripts/
|   `-- prepare-sites-build.mjs Packages the optional Sites build output
|-- server/
|   `-- content-api.mjs         Legacy local JSON development API
|-- src/
...
|   |-- App.jsx                 Routes, transitions, providers, and lazy private pages
|   |-- components.jsx          Shared navigation, footer, layout, and contact components
|   |-- content-utils.js        Date parsing and newest-first sorting helpers
...
|-- index.html                  Metadata and React root element
...
|-- vite.config.mjs             Vite development and production configuration
`-- vitest.config.mjs           React test environment configuration
```

JSON files and binary images/PDFs cannot contain source-code comments without becoming invalid or corrupted. Their purpose is documented in this structure instead. Generated files such as `package-lock.json` should be changed only through npm commands.
